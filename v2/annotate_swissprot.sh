...
#SBATCH --output=reports/diamond_sprot_out
#SBATCH --error=reports/diamond_sprot_err
...
for file in $(ls $pred_dir); do
    diamond blastp \
        --query $pred_dir/$file \
        --db $WORK/bird_bags/db/swissprot_230517/swissprot.gz \
        --out $out_dir/sprot/$file.vs.sprot.diamond \
        --threads 32 \
        --evalue 1e-04 \
